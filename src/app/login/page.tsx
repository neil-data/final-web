...
import { hasAuthToken, loginStudent } from '@/lib/auth';
...
const mapFirebaseAuthError = (err: unknown): string => {
  if (!(err instanceof FirebaseError)) {
    if (err instanceof BackendApiError) return err.message;
    if (err instanceof TypeError) {
      return 'Unable to reach backend API. Make sure backend server is running on http://127.0.0.1:8000.';
    }
  ...
  switch (err.code) {
    case 'auth/unauthorized-domain':
      return 'Firebase blocked this domain. Add localhost in Firebase Console → Authentication → Settings → Authorized domains.';
    ...
    default:
      return `Google sign-in failed (${err.code}).`;
  }
};
...
export default function LoginPage() {
  ...
  const signInStudent = async (signInEmail: string, profileName?: string, avatarUrl?: string) => {
    ...
    const nextPath = '/dashboard/student/overview';
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      await signInStudent(googleEmail, googleName, result.user.photoURL || '');
    } catch (err) {
      if (err instanceof BackendApiError) {
        setError(err.message);
      } else {
            ...
            >
              ...
              >
                <span className="flex items-center justify-center gap-2">
                  {submitLoading ? 'Signing in...' : mode === 'register' ? 'Complete Registration' : 'Sign In to Dashboard'}
                  <ArrowRight size={13} />
                </span>
              </button>
