...
import { fetchUserById, updateUserById } from '@/lib/module5Api';
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!active || me.session.role !== 'student' || !me.session.userId) return;

        const user = await fetchUserById(me.session.userId);
        if (!active) return;
