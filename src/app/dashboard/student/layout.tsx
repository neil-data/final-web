...
import { clearAuthState, fetchMe, logout } from '@/lib/auth';
  ...
  useEffect(() => {
    ...
    const validateSession = async () => {
      try {
        ...
        if (data.session.role !== 'student' || data.user?.banned) {
          clearAuthState();
          router.replace('/login');
          return;
        }

        const currentSessionRaw = localStorage.getItem('gdgoc-student-session');
        const currentSession = currentSessionRaw ? JSON.parse(currentSessionRaw) as {
          iarNo?: string;
          department?: string;
          year?: string;
        } : null;

        localStorage.setItem('gdgoc-student-session', JSON.stringify({
          name: data.user?.name || data.session.email.split('@')[0],
          email: data.session.email,
          iarNo: currentSession?.iarNo || '',
