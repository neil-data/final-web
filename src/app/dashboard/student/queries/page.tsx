...
import { ContactQuery, fetchQueries } from '@/lib/queriesApi';
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!active || me.session.role !== 'student') {
          setQueries([]);
          return;
        }
        const mine = await fetchQueries({ email: me.session.email });
        if (!active) return;
        setQueries(mine.sort((a, b) => +new Date(b.submittedAt) - +new Date(a.submittedAt)));
      } catch {
        if (!active) return;
        setQueries([]);
      }
