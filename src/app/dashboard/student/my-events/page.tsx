...
import { fetchEvents, fetchRegistrations } from '@/lib/eventsApi';
...
export default function StudentMyEventsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [allEvents, me] = await Promise.all([fetchEvents(), fetchMe()]);
        if (!active) return;
        setEvents(allEvents);

        if (me.session.role === 'student') {
          const regs = await fetchRegistrations({ email: me.session.email });
          if (!active) return;
          setRegisteredEventIds([...new Set(regs.map(reg => reg.eventId))]);
        }
      } catch {
        if (!active) return;
        setEvents([]);
  ...
  const upcomingEvents = useMemo(
    () => events.filter(e => e.status !== 'completed' && !registeredEventIds.includes(e.id)).slice(0, 6),
    [events, registeredEventIds]
  );
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          {upcomingEvents.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center glass-card rounded-xl border border-white/5">
              <Calendar size={24} className="text-white/10 mb-3" />
              <p className="text-white/30 text-sm font-mono">No upcoming events yet.</p>
              <p className="text-white/20 text-xs font-mono mt-1">Events will appear here after admin creates them.</p>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 gap-4">
