...
import { clearAuthState, fetchMe, isAdminRole, logout } from '@/lib/auth';
  ...
  useEffect(() => {
    ...
    const validateSession = async () => {
      try {
        ...
        setCurrentRole(role);
        localStorage.setItem('gdgoc-admin-session', JSON.stringify({
          email: data.session.email,
          role,
          name: data.user?.email?.split('@')[0] || data.session.email.split('@')[0],
        }));
      } catch {
        if (!active) return;
        clearAuthState();
