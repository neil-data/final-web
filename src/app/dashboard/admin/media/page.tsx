...
import { createMedia, deleteMedia, fetchMedia } from '@/lib/mediaTeamApi';
...
export default function AdminMediaPage() {
  ...
  const refresh = async () => {
    try {
      const list = await fetchMedia('all');
      setItems(list);
    } catch (err) {
      setNotice(err instanceof Error ? err.message : 'Unable to load media.');
    }
  };
  ...
  const handleAddMedia = async () => {
    ...
    try {
      ...
      setNotice(err instanceof Error ? err.message : 'Unable to add media.');
      return;
    }
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      setNotice(err instanceof Error ? err.message : 'Unable to delete media item.');
                    }
                  }}
