...
export default function AdminEventsPage() {
  ...
  const buildRegistrationsWithUsers = (
    rawRegistrations: Array<Omit<EventRegistrationWithUser, 'user'> & { user?: StudentUser }>,
    usersList: StudentUser[]
  ): EventRegistrationWithUser[] => {
    return rawRegistrations
      .map(registration => {
        const user = usersList.find(item => item.id === registration.userId);
        if (!user) return null;
        return {
          ...registration,
          user,
  ...
  const refreshData = async () => {
    try {
      const [eventList, usersList] = await Promise.all([fetchEvents(), fetchUsers()]);
      setEvents(eventList);
      setUsers(usersList);

      if (selectedEventId) {
        const regs = await fetchRegistrations({ eventId: selectedEventId });
        setRegistrations(buildRegistrationsWithUsers(regs, usersList));
      } else {
        setRegistrations([]);
      }
    } catch (err) {
      showNotice(err instanceof Error ? err.message : 'Unable to load event data.');
    }
  };
  ...
  const handleCreateEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showNotice(err instanceof Error ? err.message : 'Unable to create event.');
      return;
    }
  ...
  const handleOpenEvent = async (eventId: string) => {
    ...
    try {
      const [regs, usersList] = await Promise.all([fetchRegistrations({ eventId }), fetchUsers()]);
      setRegistrations(buildRegistrationsWithUsers(regs, usersList));
    } catch (err) {
      showNotice(err instanceof Error ? err.message : 'Unable to load registrations.');
    }
  };
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      showNotice(err instanceof Error ? err.message : 'Unable to add user.');
      return;
    }
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            showNotice(err instanceof Error ? err.message : 'Unable to delete event.');
                            return;
                          }
            ...
            <div className="mb-6 border border-white/10 rounded-lg p-4">
              ...
              <button
                onClick={async () => {
                  ...
                  const team = teams.find(item => item.teamId === teamTarget);
                  if (!team) {
                    showNotice('Team not found.');
                    return;
                  }
                  ...
                  try {
                    await createRegistration({
                      eventId: selectedEventId,
                      userId: '',
                      name: teamMemberName.trim() || teamMemberEmail.split('@')[0],
                      email: teamMemberEmail.trim().toLowerCase(),
                      iarNo: '',
                      department: '',
                      year: '',
                      teamId: team.teamId,
                    ...
                    showNotice(err instanceof Error ? err.message : 'Unable to add member.');
                    return;
                  }
                      ...
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={async () => {
                            ...
                            try {
                              await updateRegistration(reg.id, { name, email, iarNo, department, year });
                            } catch (err) {
                              showNotice(err instanceof Error ? err.message : 'Unable to update details.');
                              return;
                            }
                        ...
                        <button onClick={async () => {
                          try {
                            await updateUserPoints(reg.user.id, -50);
                            await refreshData();
                          } catch (err) {
                            showNotice(err instanceof Error ? err.message : 'Unable to update points.');
                          }
                        }} className="w-7 h-7 rounded border border-white/10 flex items-center justify-center text-white/40 hover:text-g-red hover:border-g-red/30 transition-colors"><Minus size={11} /></button>
                        <button onClick={async () => {
                          try {
                            await updateUserPoints(reg.user.id, 50);
                            await refreshData();
                        ...
                        }} className="w-7 h-7 rounded border border-white/10 flex items-center justify-center text-white/40 hover:text-g-green hover:border-g-green/30 transition-colors"><PlusCircle size={11} /></button>
                        <button
                          onClick={async () => {
                            try {
                              ...
                              showNotice(err instanceof Error ? err.message : 'Unable to remove.');
                              return;
                            }
                          }}
                        ...
                        <button onClick={async () => {
                          try {
                            await updateUserBannedStatus(reg.user.id, true);
                            await refreshData();
                          } catch (err) {
                            showNotice(err instanceof Error ? err.message : 'Unable to ban user.');
                          }
                        }} className="w-7 h-7 rounded border border-white/10 flex items-center justify-center text-white/40 hover:text-g-red hover:border-g-red/30 transition-colors"><Ban size={11} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
