...
import { createAnnouncement, deleteAnnouncement, fetchAnnouncements } from '@/lib/module5Api';
...
export default function AdminAnnouncementsPage() {
  ...
  const refresh = async () => {
    try {
      ...
      setNotice(err instanceof Error ? err.message : 'Unable to load announcements.');
    }
  };
  ...
  const post = async () => {
    ...
    try {
      await createAnnouncement({
        title: newTitle,
        content: newContent,
        type: newType as Announcement['type'],
        pinned: false,
      });
      await refresh();
    } catch (err) {
      setNotice(err instanceof Error ? err.message : 'Unable to post announcement.');
      return;
    }
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    setNotice(err instanceof Error ? err.message : 'Unable to delete announcement.');
                  }
                }}
