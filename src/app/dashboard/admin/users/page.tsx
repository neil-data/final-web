...
import { createTeamMember, deleteTeamMember, fetchTeamMembers, updateTeamMember } from '@/lib/mediaTeamApi';
import { fetchUsers, StudentUser, updateUserBannedStatus, updateUserPoints } from '@/lib/eventsApi';
import {
  createStudentUser,
  deleteUserById,
} from '@/lib/module5Api';
...
export default function AdminUsersPage() {
  ...
  const refreshUsers = async () => {
    try {
      ...
      setNotice(err instanceof Error ? err.message : 'Unable to load users.');
    }
  };
  const refreshMembers = async () => {
    try {
      const data = await fetchTeamMembers('all');
      setMembers(data);
      setNotice('');
    } catch (err) {
      setNotice(err instanceof Error ? err.message : 'Unable to load team members.');
    }
  };
  ...
  const handleTeamSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotice(err instanceof Error ? err.message : 'Unable to save team member.');
      return;
    }
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotice(err instanceof Error ? err.message : 'Unable to add user.');
    }
  };
                ...
                <motion.tr key={user.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}>
                  ...
                  <td>
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={async () => { try { await updateUserPoints(user.id, -50); await refreshUsers(); } catch (err) { setNotice(err instanceof Error ? err.message : 'Unable to update points.'); } }} className="w-7 h-7 rounded border border-white/10 flex items-center justify-center text-white/40 hover:text-g-red hover:border-g-red/30 transition-colors"><Minus size={11} /></button>
                      <button onClick={async () => { try { await updateUserPoints(user.id, 50); await refreshUsers(); } catch (err) { setNotice(err instanceof Error ? err.message : 'Unable to update points.'); } }} className="w-7 h-7 rounded border border-white/10 flex items-center justify-center text-white/40 hover:text-g-green hover:border-g-green/30 transition-colors"><PlusCircle size={11} /></button>
                      {user.banned ? (
                        <button onClick={async () => { try { await updateUserBannedStatus(user.id, false); await refreshUsers(); } catch (err) { setNotice(err instanceof Error ? err.message : 'Unable to update user status.'); } }} className="w-7 h-7 rounded border border-white/10 flex items-center justify-center text-white/40 hover:text-g-blue hover:border-g-blue/30 transition-colors"><ShieldCheck size={11} /></button>
                      ) : (
                        <button onClick={async () => { try { await updateUserBannedStatus(user.id, true); await refreshUsers(); } catch (err) { setNotice(err instanceof Error ? err.message : 'Unable to update user status.'); } }} className="w-7 h-7 rounded border border-white/10 flex items-center justify-center text-white/40 hover:text-g-red hover:border-g-red/30 transition-colors"><Ban size={11} /></button>
                      )}
                      <button onClick={async () => { try { await deleteUserById(user.id); await refreshUsers(); } catch (err) { setNotice(err instanceof Error ? err.message : 'Unable to delete user.'); } }} className="w-7 h-7 rounded border border-white/10 flex items-center justify-center text-white/40 hover:text-g-red hover:border-g-red/30 transition-colors"><Trash2 size={11} /></button>
                    </div>
                  </td>
                </motion.tr>
              ))}
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={async () => {
                        try {
                          ...
                          setNotice(err instanceof Error ? err.message : 'Unable to delete team member.');
                        }
                      }} className="w-7 h-7 rounded border border-white/10 flex items-center justify-center text-white/40 hover:text-g-red hover:border-g-red/30 transition-colors"><Trash2 size={12} /></button>
                    </div>
                  </td>
                </motion.tr>
              ))}
