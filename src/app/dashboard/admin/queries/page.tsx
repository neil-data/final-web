...
import { ContactQuery, fetchQueries, replyToQuery } from '@/lib/queriesApi';
...
export default function AdminQueriesPage() {
  ...
  const refresh = async () => {
    try {
      const next = await fetchQueries();
      setQueries(next.sort((a, b) => +new Date(b.submittedAt) - +new Date(a.submittedAt)));
      setNotice('');
    } catch (err) {
      setQueries([]);
      setNotice(err instanceof Error ? err.message : 'Unable to load queries.');
    }
  };
  ...
  const handleReply = async (e: FormEvent) => {
    ...
    try {
      await replyToQuery(selectedQuery.id, replyText);
    } catch (err) {
