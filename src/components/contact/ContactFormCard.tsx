...
import { createQuery } from '@/lib/queriesApi';
...
export function ContactFormCard() {
  ...
  useEffect(() => {
    ...
    const prefill = async () => {
      try {
        ...
        if (!active || me.session.role !== 'student') return;
        setName(me.user?.name || me.session.email.split('@')[0]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const text = err instanceof Error ? err.message : 'Unable to send your message right now.';
