...
import { createRegistration, deleteRegistration } from '@/lib/eventsApi';
...
export default function RegistrationModal({ eventId, event, eventTitle, eventDate, isOpen, onClose, onRegistered }: RegistrationModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      sessionUserId = me.session.userId || '';
    } catch {
    ...
    try {
      const teamId = teamEnabled ? `team-${Date.now()}-${Math.random().toString(36).slice(2, 7)}` : undefined;
      ...
      const leader = await createRegistration({
        ...
        year: formData.year,
        teamId,
        teamName: teamEnabled ? teamName.trim() : undefined,
        isLeader: teamEnabled,
        customFieldValues,
      });
      createdRegistrationIds.push(leader.id);
