...
export default function Navbar() {
  ...
  useEffect(() => {
    const syncAuthState = () => {
      ...
      const studentSessionRaw = localStorage.getItem('gdgoc-student-session');

      setAdminLoggedIn(Boolean(token && adminSessionRaw));

      if (token && studentSessionRaw) {
        try {
          const session = JSON.parse(studentSessionRaw) as { name?: string; avatar?: string };
          setStudentLoggedIn(true);
          setStudentName((session.name || '').trim());
          setStudentAvatar((session.avatar || '').trim());
        } catch {
          setStudentLoggedIn(true);
          setStudentName('');
          setStudentAvatar('');
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              {studentLoggedIn ? (
                <Link
                  ...
                  className="flex items-center gap-2.5 rounded-full border border-white/10 bg-white/[0.03] pl-2 pr-3 py-1.5 hover:border-g-blue/40 transition-colors"
                >
                  {studentAvatar ? (
                    <Image
                      src={studentAvatar}
                    ...
                    <div className="w-6 h-6 rounded-full border border-white/10 bg-g-blue/20 text-[10px] font-mono text-g-blue flex items-center justify-center uppercase">
                      {(studentName || 'S').slice(0, 1)}
                    </div>
                  )}
                  <span className="text-[11px] font-mono uppercase tracking-widest text-white/70 hover:text-white transition-colors">
                    {studentName ? studentName.split(' ')[0] : 'Dashboard'}
                  </span>
                </Link>
              ) : (
                <>
